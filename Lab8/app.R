...
server <- function(input, output) {
  ...
  dTh <- reactive({
    if (input$theoreticalDist == 'beta') {
      dat <- rbeta(n, input$qqShapeBeta, qqScaleBeta)
      distr <- 'beta'
      params <- c(input$qqShapeBeta, qqScaleBeta)
  ...
  vTh <- reactive({
    if (input$theoreticalDist2 == 'beta') {
      dat <- rbeta(n, input$qqShapeBeta2, qqScaleBeta2)
      distr <- 'beta'
      params <- c(input$qqShapeBeta2, qqScaleBeta2)
